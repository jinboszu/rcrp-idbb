...
static void usage() {
  fprintf(stdout, "usage: dis-solve -h\n");
  fprintf(stdout, "usage: dis-solve"
                  " -i $input_file"
                  " -t $time_limit\n");
  fprintf(stdout, "\t-i: input file\n");
  fprintf(stdout, "\t-t: time limit in seconds\n");
...
int main(int argc, char **argv) {
  ...
  for (int opt; (opt = getopt_long(argc, argv, optstr, options, NULL)) != -1;) {
    switch (opt) {
    case 'h':
      usage();
      return EXIT_SUCCESS;
  ...
  instance_t *inst = read_zqlz_instance(infile);
  if (inst == NULL) {
