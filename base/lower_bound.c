...
static void enumerate(int *priority, int next, int n_bad, int *quality, int len,
                      int curr, int *best) {
  if (next == n_bad) {
    *best = curr;
    return;
  ...
  if (pos < len) {
    ...
    enumerate(priority, next + 1, n_bad, quality, len, curr, best);
    quality[pos] = backup;
  }

  if (pos > 0 && curr + 1 < *best) {
    enumerate(priority, next + 1, n_bad, quality, len, curr + 1, best);
  }
}
...
int lb4(state_t *state, int *h, int *list, int *quality, int *priority) {
  if (state->n_bad == 0) {
    return state->n_bad;
  }
  ...
  while (remain > 0) {
    ...
    if (n_bad > 1) {
      ...
      int best = n_bad - 1;
      enumerate(priority, 0, n_bad, quality, len, 0, &best);
      k += best;
    }
